                            ...
                            <tbody id="tbody-data">
                                <?php foreach ($data['list'] as $item) { ?>
                                    <tr>
                                        ...
                                        <td>
                                            <form action="<?=route('admin.certificate_delete') ?>" method="post" id="delete-item-form">
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Thêm mới hồ sơ trả về<i></i></h5>
            ...
            <div class="modal-body">
                <form action="<?=route('admin.certificate_store')?>" method="POST">
                    <div class="form-group">
                        <label for="exampleFormControlFile1">Thông tin hồ sơ trả về </label>
                        <input type="text" name="name" class="form-control" placeholder="Nhập thông tin hồ sơ trả về" required>
                    </div>
...
<div class="modal fade" id="exampleModal1" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Chỉnh sửa Hồ sơ - Văn bản cần trả<i></i></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
<script>
    ...
    $(document).on('click', '#edit', function(){
        ...
        $.ajax({
            type: 'GET',
            url: '<?=route('admin.certificate_show') . '?id='?>' + id
        }).done((res)=>{
            let data = JSON.parse(res);
            if (data.status) {
                let form = `
                <form action="<?=route('admin.certificate_update')?>" method="POST">
                    <div class="form-group">
